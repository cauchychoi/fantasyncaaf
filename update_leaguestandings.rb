...
matchupSchedules.each do |matchup|
	...
	#homeScore = client.query("SELECT pointsScored FROM leaguescores WHERE week="+currentWeek+" AND teamID="+matchup["homeTeamID"])
	#awayScore = client.query("SELECT pointsScored FROM leaguescores WHERE week="+currentWeek+" AND teamID="+matchup["awayTeamID"])
