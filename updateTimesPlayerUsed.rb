...
tuesdayAfterWeek1 = Time.new(2023,9,5,7,0,0,"+00:00")
			...
			if (player["hasPlayed"] == 0)				
				if (player["position"].eql?("DEF"))
					unless defenseProcessed.include?(player["fantasyID"])
						if !(currentWeek == 12 && (player["fantasyID"] == 101 || player["fantasyID"] == 51))  # Bonus game logic, won't run in week 12 for Cauchy and Jon
