...
task :update_scores do
	...
	#test
	#if (Time.now.utc..Time.now.utc+(7*60*60)).cover?(Time.now.utc+(2*60*60))
	#	puts "YAY"
	#end
...
task :update_leaguestandings do
	if Time.now.utc.wday == 1 && (Time.new(2018,8,29,7,0,0,"+00:00")..Time.new(2018,11,13,7,0,0,"+00:00")).cover?(Time.now.utc)
