...
roster.each do |player|
	...
	if (!teamName.to_s.eql?("") && !playerName.to_s.eql?("") && !position.to_s.eql?("") && !playerID.to_s.eql?("") && !position.to_s.eql?("DB") && !position.to_s.eql?("LB") && !position.to_s.eql?("G") && !position.to_s.eql?("LS") && !position.to_s.eql?("OL") && !position.to_s.eql?("DT") && !position.to_s.eql?("S") && !position.to_s.eql?("CB") && !position.to_s.eql?("P") && !position.to_s.eql?("C") && !position.to_s.eql?("DL") && !position.to_s.eql?("DE") && !position.to_s.eql?("OT"))
		client.query("INSERT INTO collegeTeamRoster (team, PlayerName, position, playerID) VALUES ('#{teamName}', '#{playerName}', '#{position}', '#{playerID}') ON DUPLICATE KEY UPDATE team=VALUES(team), PlayerName=VALUES(PlayerName), position=VALUES(position)")
